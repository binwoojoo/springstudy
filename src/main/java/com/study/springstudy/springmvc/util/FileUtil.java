...
public class FileUtil {
    ...
    public static String uploadFile(String rootPath, MultipartFile file) {
        ...
        // 파일 전체 경호
        // fullPath = D:/spring_prj/upload/2024/06/05/sadaslkdnsalkdna dog.jpg
        String fullPath = newUploadPath + "/" + newFileName;

        // url-path: /local/2024/06/05/sadaslkdnsalkdna dog.jpg
        String urlPath = "/local" + fullPath.substring(rootPath.length());

        // 업로드가 완료되면 데이터 베이스에 파일의 경로 위치를 저장
        // ex) /local/2024/06/05/asdsadasdasdasdsa dog.jpg
